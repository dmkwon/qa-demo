        ...
        <Masonry columns={{ xs: 1, md: 2 }} spacing={3}>
          <ComponentBlock title="Image">
            {[1, 2, 3, 4, 5].map((_, index) => (
              <Avatar key={index} alt={_mock.fullName(index + 1)} src={null} />
            ))}
          </ComponentBlock>
            ...
            <Stack spacing={2} alignItems="center">
              {SIZES.map((size) => (
                <Tooltip key={size} title={size}>
                  ...
                  >
                    {COLORS.map((color, index) => (
                      <Avatar key={color} alt={_mock.fullAddress(index + 1)} src={null} />
                    ))}
                  </AvatarGroup>
                </Tooltip>
              ))}
                ...
                <Badge variant="online" anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}>
                  <AvatarGroup variant="compact" sx={{ width: 48, height: 48 }}>
                    {COLORS.slice(0, 2).map((color, index) => (
                      <Avatar key={color} alt={_mock.fullName(index + 1)} src={null} />
                    ))}
                  </AvatarGroup>
                </Badge>
              </Tooltip>
          ...
          <ComponentBlock title="With Badge">
            ...
            >
              <Avatar alt={_mock.fullName(8)} src={null} />
            </Badge>
            ...
            {STATUS.map((status, index) => (
              <Badge
                ...
                anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
              >
                <Avatar alt={_mock.fullName(index + 1)} src={null} />
              </Badge>
            ))}
          </ComponentBlock>
