...
import DataManagerInvitedItem from './data-manager-invited-item';

// ----------------------------------------------------------------------

export default function DataManagerFileDetails({
  item,
  open,
  favorited,
  //
  ...
  const renderShared = (
    <>
      ...
      {hasShared && (
        <Box sx={{ pl: 2.5, pr: 1 }}>
          {shared.map((person) => (
            <DataManagerInvitedItem key={person.id} person={person} />
          ))}
        </Box>
      )}
    </>
...
DataManagerFileDetails.propTypes = {
